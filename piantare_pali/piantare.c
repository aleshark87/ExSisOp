...
void* martello0(void){
	...
	while(1){
		...
		pthread_mutex_lock(&P->mutex); P->hoFatto[indice] = 1; 
...
void* martello1(void){
	int indice; int paliMartellati; indice = 1; paliMartellati = 0;
	while(1){
		paliMartellati++;
		pthread_mutex_lock(&P->mutex);
		while(P->deviMartellare[indice] == 0){
...
void* tieniPali(void){
	...
	while(1){
		...
		sleep(2);
		pthread_mutex_lock(&P->mutex);
		P->deviMartellare[0] = 1; P->deviMartellare[1] = 1;
		pthread_cond_signal(&P->wait0); pthread_cond_signal(&P->wait1);
		while(P->hoFatto[0] == 0 || P->hoFatto[1] == 0){
			pthread_cond_wait(&P->aspetto, &P->mutex);
		}
		P->hoFatto[0] = 0; P->hoFatto[1] = 0;
		pthread_mutex_unlock(&P->mutex);
		if(coppiePaliPresi == 5) { sleep(3); } 
	}
}
...
int main(void){
	...
	P->deviMartellare[0] = 0; P->deviMartellare[1] = 0; P->hoFatto[0] = 0; 
	P->hoFatto[1] = 0;
