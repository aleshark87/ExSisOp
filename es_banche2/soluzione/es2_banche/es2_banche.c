...
{ 
	...
	while(1) 
	{
		...
		sleep(1);
		DBGpthread_mutex_lock(&mutex[indicebanca],Clabel); 
		T[indicebanca] += 10.0;
		N[indicebanca] ++;
		...
		DBGpthread_mutex_unlock(&mutex[indicebanca],Clabel); 
	}
	pthread_exit(NULL); 
} 
...
{ 
	...
	while(1) 
	{
		...
		for( i=0; i<NUMBANCHE; i++ )
			DBGpthread_mutex_lock(&mutex[i],Clabel); 
		...
		for( i=NUMBANCHE-1; i>=0; i-- )
			DBGpthread_mutex_unlock(&mutex[i],Clabel); 
