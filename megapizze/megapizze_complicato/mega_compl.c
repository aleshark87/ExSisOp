...
void* pizza_lover(int arg){
	...
	while(1){
		...
		P->personeAlTavolo++;
		if(P->personeAlTavolo == 3){
			printf("[%d]FORZA PIZZAIOLO ! \n", indiceCl);
			P->forzaPizzaiolo = 1;
			DBGpthread_cond_broadcast(&P->waitPizzaiolo);
		...
		while(P->pizza == 0){
			printf("[%d]Aspetto la pizza...\n", indiceCl);
			DBGpthread_cond_wait(&P->waitPizza, &P->mutex);
		}
		printf("[%d]FINALMENTE SI MANGIA CAZZO ! \n", indiceCl);
		DBGpthread_mutex_unlock(&P->mutex);
		sleep(3);
		DBGpthread_mutex_lock(&P->mutex);
		P->personeAlTavolo--; P->mangiando = 0; P->pizza = 0;
...
void* pizzaiolo(int arg){
	while(1){
		DBGpthread_mutex_lock(&P->mutex);
		while(P->forzaPizzaiolo == 0){
			DBGpthread_cond_wait(&P->waitPizzaiolo, &P->mutex);
		}
		DBGpthread_mutex_unlock(&P->mutex);
		printf("PREPARO LA PIZZA !! \n");
		sleep(1);
		DBGpthread_mutex_lock(&P->mutex);
		P->forzaPizzaiolo = 0; P->pizza = 1;
...
int main(void){
	...
	if( rc ) PrintERROR_andExit(rc,"pthread_cond_init failed");
	rc = pthread_cond_init(&P->waitPizzaiolo, &cvattr );
	if( rc ) PrintERROR_andExit(rc,"pthread_cond_init failed");
	rc = pthread_mutex_init(&P->mutex, &mattr ); 
	if( rc ) PrintERROR_andExit(rc,"pthread_mutex_init failed");

	P->mangiando = 0; P->personeAlTavolo = 0 ;
	P->forzaPizzaiolo = 0 ;
	P->pizza = 0;
